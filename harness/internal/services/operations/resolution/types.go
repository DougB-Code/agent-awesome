// This file defines Operation input resolution data contracts.
package resolution
...
const (
	...
	// SourceOperationDefault identifies values saved on the Operation.
	SourceOperationDefault Source = "operation_default"
	// SourceCodebaseDefault identifies values from a codebase catalog entry.
	SourceCodebaseDefault Source = "codebase_default"
	// SourceWorkflowDefault identifies values from the workflow definition.
	SourceWorkflowDefault Source = "workflow_default"
	...
	// SourceStepOutput identifies values produced by earlier workflow steps.
...
// Request asks the resolver to assemble complete workflow input.
type Request struct {
	RequiredFields    []string
	RunRequest        map[string]any
	OperationDefaults map[string]any
