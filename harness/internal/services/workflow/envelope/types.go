// This file defines the AA workflow envelope data model.
package envelope
...
const envelopeSchemaRef = "aa.workflow.envelope.v1"

// Envelope is the universal workflow node input and output carrier.
...
type Metadata struct {
	...
	WorkflowRunID   string           `json:"workflow_run_id,omitempty" yaml:"workflow_run_id,omitempty"`
...
// Control stores deterministic workflow status hints.
type Control struct {
...
func New(workflowRunID string, nodeRunID string, attempt int, bodyValue any) Envelope {
	return Envelope{
		Meta: Metadata{
			SchemaRef:     envelopeSchemaRef,
			WorkflowRunID: strings.TrimSpace(workflowRunID),
			NodeRunID:     strings.TrimSpace(nodeRunID),
			CorrelationID: strings.TrimSpace(workflowRunID),
			Attempt:       attempt,
...
func Empty(workflowRunID string, nodeRunID string, attempt int) Envelope {
	env := New(workflowRunID, nodeRunID, attempt, nil)
...
func NormalizeResult(workflowRunID string, nodeRunID string, attempt int, result map[string]any, status string) Envelope {
	env := FromMap(result)
	if strings.TrimSpace(env.Meta.SchemaRef) == "" || env.Meta.SchemaRef != envelopeSchemaRef {
		env = New(workflowRunID, nodeRunID, attempt, cloneAny(result))
	}
	env.Meta.WorkflowRunID = strings.TrimSpace(workflowRunID)
