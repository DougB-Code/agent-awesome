// This file implements the action host boundary used by workflow actions.
package runtime
...
func (s *Service) RequestHuman(ctx context.Context, req actions.HumanRequest) (string, error) {
	...
	_ = s.appendEvent(ctx, req.RunID, "human_requested", "workflow is waiting for user input", map[string]any{"pending_id": id, "step_id": req.StepID})
	return id, nil
}
...
func (s *Service) GenerateLLM(ctx context.Context, req actions.LLMRequest) (map[string]any, error) {
	...
	result, err := s.llm.GenerateWorkflowJSON(ctx, req)
	if err != nil {
		return nil, err
	}
	if result == nil {
