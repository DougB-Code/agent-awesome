...
func (s *Service) executeState(ctx context.Context, def definition.Definition, run store.RunRecord, states map[string]definition.StateDefinition, stateID string, depth int) (string, error) {
	...
	if err := s.executeStateEntry(ctx, run, state); err != nil {
...
// executeStateEntry executes all entry actions for one state in declared order.
func (s *Service) executeStateEntry(ctx context.Context, run store.RunRecord, state definition.StateDefinition) error {
	for _, actionNode := range state.OnEntry {
		...
		if err := s.executeStateAction(ctx, run, actionNode); err != nil {
			return err
		}
	}
	return nil
...
// executeStateAction invokes one state entry action and stores its envelope output.
func (s *Service) executeStateAction(ctx context.Context, run store.RunRecord, node definition.NodeDefinition) error {
	attempts := nodeStateAttempts(ctx, s.store, run.ID, node.ID) + 1
	...
	output, err := s.actions.Execute(ctx, definition.NodeAction(node), actions.Context{
		RunID:  run.ID,
		StepID: node.ID,
		Input:  input,
		Host:   s,
	...
	if err != nil {
		if errors.Is(err, actions.ErrPending) {
			...
			_ = s.store.SaveStepOutput(ctx, run.ID, node.ID+".pending", env.ToMap())
		...
		outputMap = env.ToMap()
		_ = s.store.SaveStepOutput(ctx, run.ID, node.ID, outputMap)
		if fireErr := s.fireNodeStateTrigger(ctx, run.ID, node.ID, nodeTriggerFail, attempts, nil, err.Error()); fireErr != nil {
			return fireErr
		}
		_ = s.appendEvent(ctx, run.ID, "step_failed", err.Error(), map[string]any{"step_id": node.ID})
		return err
	}
