// This file validates and persists workflow authoring design artifacts.
package runtime
...
func (s *Service) SuggestDesignArtifacts(ctx context.Context, req DesignSuggestionRequest) (DesignSuggestionResult, error) {
	if s.cfg.DesignAssistant == nil {
		return DesignSuggestionResult{}, fmt.Errorf("workflow design assistant is not configured")
	}
...
func (s *Service) validateDesignArtifact(kind string, body map[string]any) error {
	...
	case "workflow_explanation":
		var artifact WorkflowExplanationArtifact
		if err := json.Unmarshal(encoded, &artifact); err != nil {
			return fmt.Errorf("decode workflow explanation artifact: %w", err)
		}
		if strings.TrimSpace(artifact.Summary) == "" {
			return fmt.Errorf("workflow explanation summary is required")
		}
	case "workflow":
		var def definition.Definition
		if err := json.Unmarshal(encoded, &def); err != nil {
			return fmt.Errorf("decode workflow artifact: %w", err)
		}
		if err := definition.Validate(def, s.actions); err != nil {
			return fmt.Errorf("workflow artifact is invalid: %w", err)
		}
	default:
