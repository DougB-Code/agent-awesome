// This file defines ordered workflow decision rules.
package decision
...
func MatchWhen(when When, input envelope.Envelope) (bool, error) {
	...
	if strings.TrimSpace(when.Expr) != "" {
		emptyOutput := envelope.Empty(input.Meta.WorkflowRunID, input.Meta.NodeRunID, input.Meta.Attempt)
		vars := workflowexpr.VariablesFromEnvelopes(input, emptyOutput, nil)
		return workflowexpr.EvaluateBool(when.Expr, vars)
	}
	if strings.TrimSpace(when.Path) != "" {
		value, ok := lookupPath(input, when.Path)
		return ok && workflowexpr.Truthy(value), nil
	}
	return true, nil
}
