...
//   - Serve the UI and Operations service with target records without coupling
//     workflow orchestration to one machine or deployment shape.
//
// High-level example:
//   - The embedded harness starts, records the local target with its capability
//     ids, and the UI lists that target as the default place Operations can run.
package targets
