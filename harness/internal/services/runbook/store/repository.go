// This file implements runbook repository launchpad.
package store
...
// UpsertDefinition stores or replaces one loaded runbook definition.
func (s *Store) UpsertDefinition(ctx context.Context, record DefinitionRecord) error {
	...
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_definitions (id, kind, name, hash, body_json, updated_at)
		VALUES (?, ?, ?, ?, ?, ?)
	...
	if err != nil {
		return fmt.Errorf("upsert runbook definition %q: %w", record.ID, err)
	}
	return nil
}
...
func (s *Store) ListDefinitions(ctx context.Context) ([]DefinitionRecord, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, kind, name, hash, body_json, updated_at FROM runbook_definitions ORDER BY id`)
	if err != nil {
		return nil, fmt.Errorf("list runbook definitions: %w", err)
	}
	defer rows.Close()
...
func (s *Store) DeleteDefinitionsExcept(ctx context.Context, ids []string) error {
	if len(ids) == 0 {
		if _, err := s.db.ExecContext(ctx, `DELETE FROM runbook_definitions`); err != nil {
			return fmt.Errorf("delete runbook definitions: %w", err)
		}
		return nil
	}
	...
	query := `DELETE FROM runbook_definitions WHERE id NOT IN (` + strings.Join(placeholders, ",") + `)`
	if _, err := s.db.ExecContext(ctx, query, args...); err != nil {
		return fmt.Errorf("delete stale runbook definitions: %w", err)
	}
	return nil
}

// CreateRun inserts one new runbook run.
func (s *Store) CreateRun(ctx context.Context, record RunRecord) error {
	...
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_runs (id, definition_id, kind, status, state, input_json, output_json, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		record.ID, record.DefinitionID, record.Kind, record.Status, record.State, string(input), string(output), now, now)
	if err != nil {
		return fmt.Errorf("create runbook run %q: %w", record.ID, err)
	}
	return nil
}

// GetRun loads one runbook run by id.
func (s *Store) GetRun(ctx context.Context, id string) (RunRecord, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, definition_id, kind, status, state, input_json, output_json, created_at, updated_at FROM runbook_runs WHERE id = ?`, id)
	return scanRun(row)
}
...
func (s *Store) ListRunsByStatus(ctx context.Context, status string) ([]RunRecord, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, definition_id, kind, status, state, input_json, output_json, created_at, updated_at FROM runbook_runs WHERE status = ? ORDER BY created_at`, status)
	if err != nil {
		return nil, fmt.Errorf("list runbook runs by status %q: %w", status, err)
	}
	defer rows.Close()
...
func (s *Store) UpdateRunState(ctx context.Context, id string, status string, state string, output map[string]any) error {
	...
	_, err = s.db.ExecContext(ctx, `UPDATE runbook_runs SET status = ?, state = ?, output_json = ?, updated_at = ? WHERE id = ?`, status, state, string(encoded), nowString(), id)
	if err != nil {
		return fmt.Errorf("update runbook run %q: %w", id, err)
	}
	return nil
}
...
func (s *Store) AppendEvent(ctx context.Context, runID string, eventType string, message string, data map[string]any) error {
	...
	if err != nil {
		return fmt.Errorf("encode runbook event: %w", err)
	}
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_events (run_id, type, message, data_json, created_at) VALUES (?, ?, ?, ?, ?)`, runID, eventType, message, string(encoded), nowString())
	if err != nil {
		return fmt.Errorf("append runbook event for run %q: %w", runID, err)
	}
	return nil
}
...
func (s *Store) ListEvents(ctx context.Context, runID string) ([]EventRecord, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, run_id, type, message, data_json, created_at FROM runbook_events WHERE run_id = ? ORDER BY id`, runID)
	if err != nil {
		return nil, fmt.Errorf("list runbook events for run %q: %w", runID, err)
	}
	defer rows.Close()
...
func (s *Store) SaveStepOutput(ctx context.Context, runID string, stepID string, output map[string]any) error {
	...
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_step_outputs (run_id, step_id, output_json, created_at)
		VALUES (?, ?, ?, ?)
	...
	if err != nil {
		return fmt.Errorf("save runbook step output %s/%s: %w", runID, stepID, err)
	}
	return nil
}
...
func (s *Store) StepOutput(ctx context.Context, runID string, stepID string) (map[string]any, bool, error) {
	var encoded string
	err := s.db.QueryRowContext(ctx, `SELECT output_json FROM runbook_step_outputs WHERE run_id = ? AND step_id = ?`, runID, stepID).Scan(&encoded)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, false, nil
		}
		return nil, false, fmt.Errorf("load runbook step output %s/%s: %w", runID, stepID, err)
	}
	var output map[string]any
	if err := json.Unmarshal([]byte(encoded), &output); err != nil {
		return nil, false, fmt.Errorf("decode runbook step output %s/%s: %w", runID, stepID, err)
	}
	return output, true, nil
}

// StepOutputs loads persisted step outputs for one runbook run.
func (s *Store) StepOutputs(ctx context.Context, runID string) (map[string]map[string]any, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT step_id, output_json FROM runbook_step_outputs WHERE run_id = ?`, runID)
	if err != nil {
		return nil, fmt.Errorf("list runbook step outputs %s: %w", runID, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var output map[string]any
		if err := json.Unmarshal([]byte(encoded), &output); err != nil {
			return nil, fmt.Errorf("decode runbook step output %s/%s: %w", runID, stepID, err)
		}
...
// GetNodeState loads one durable runbook node state record.
func (s *Store) GetNodeState(ctx context.Context, runID string, stateID string) (NodeStateRecord, bool, error) {
	row := s.db.QueryRowContext(ctx, `SELECT run_id, state_id, status, attempts, output_json, error, started_at, completed_at, updated_at
		FROM runbook_node_states WHERE run_id = ? AND state_id = ?`, runID, stateID)
...
// ListNodeStates loads every durable runbook node state record for one run.
func (s *Store) ListNodeStates(ctx context.Context, runID string) ([]NodeStateRecord, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT run_id, state_id, status, attempts, output_json, error, started_at, completed_at, updated_at
		FROM runbook_node_states WHERE run_id = ? ORDER BY state_id`, runID)
	if err != nil {
		return nil, fmt.Errorf("list runbook node states for run %q: %w", runID, err)
	}
	defer rows.Close()
...
func (s *Store) MarkNodeStateRunning(ctx context.Context, runID string, stateID string, attempts int) error {
	now := nowString()
	_, err := s.db.ExecContext(ctx, `INSERT INTO runbook_node_states
	...
	if err != nil {
		return fmt.Errorf("mark runbook node state running %s/%s: %w", runID, stateID, err)
	}
	return nil
}
...
func (s *Store) MarkNodeStateSucceeded(ctx context.Context, runID string, stateID string, attempts int, output map[string]any) error {
	encoded, err := json.Marshal(nilMap(output))
	if err != nil {
		return fmt.Errorf("encode runbook node output: %w", err)
	}
	now := nowString()
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_node_states
		(run_id, state_id, status, attempts, output_json, error, started_at, completed_at, updated_at)
	...
	if err != nil {
		return fmt.Errorf("mark runbook node state succeeded %s/%s: %w", runID, stateID, err)
	}
	return nil
}
...
func (s *Store) MarkNodeStateSkipped(ctx context.Context, runID string, stateID string, attempts int, output map[string]any) error {
	encoded, err := json.Marshal(nilMap(output))
	if err != nil {
		return fmt.Errorf("encode runbook skipped node output: %w", err)
	}
	now := nowString()
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_node_states
		(run_id, state_id, status, attempts, output_json, error, started_at, completed_at, updated_at)
	...
	if err != nil {
		return fmt.Errorf("mark runbook node state skipped %s/%s: %w", runID, stateID, err)
	}
	return nil
}
...
func (s *Store) MarkNodeStateFailed(ctx context.Context, runID string, stateID string, attempts int, message string) error {
	...
	if err != nil {
		return fmt.Errorf("mark runbook node state failed %s/%s: %w", runID, stateID, err)
	}
	return nil
}
...
func (s *Store) CreatePendingItem(ctx context.Context, item PendingItem) error {
	...
	now := nowString()
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_pending_items (id, run_id, step_id, status, prompt, payload_json, response_json, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		item.ID, item.RunID, item.StepID, item.Status, item.Prompt, string(payload), string(response), now, now)
	if err != nil {
		return fmt.Errorf("create pending runbook item %q: %w", item.ID, err)
	}
	return nil
}
...
func (s *Store) CompletePendingItems(ctx context.Context, runID string, response map[string]any) error {
	...
	_, err = s.db.ExecContext(ctx, `UPDATE runbook_pending_items SET status = ?, response_json = ?, updated_at = ? WHERE run_id = ? AND status = ?`, PendingStatusCompleted, string(encoded), nowString(), runID, PendingStatusOpen)
	if err != nil {
		return fmt.Errorf("complete pending runbook items for run %q: %w", runID, err)
	}
	return nil
}

// ListOpenPendingItems returns open user-visible runbook items.
func (s *Store) ListOpenPendingItems(ctx context.Context) ([]PendingItem, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, run_id, step_id, status, prompt, payload_json, response_json, created_at, updated_at FROM runbook_pending_items WHERE status = ? ORDER BY created_at`, PendingStatusOpen)
	if err != nil {
		return nil, fmt.Errorf("list runbook inbox: %w", err)
	}
	defer rows.Close()
...
func scanDefinition(row interface{ Scan(...any) error }) (DefinitionRecord, error) {
	...
	if err := json.Unmarshal([]byte(body), &record.Body); err != nil {
		return DefinitionRecord{}, fmt.Errorf("decode runbook definition body: %w", err)
	}
	return record, nil
}
...
func scanRun(row interface{ Scan(...any) error }) (RunRecord, error) {
	...
	if err := row.Scan(&record.ID, &record.DefinitionID, &record.Kind, &record.Status, &record.State, &input, &output, &record.CreatedAt, &record.UpdatedAt); err != nil {
		if err == sql.ErrNoRows {
			return RunRecord{}, fmt.Errorf("runbook run not found")
		}
		return RunRecord{}, err
	}
	if err := json.Unmarshal([]byte(input), &record.Input); err != nil {
		return RunRecord{}, fmt.Errorf("decode runbook run input: %w", err)
	}
	if err := json.Unmarshal([]byte(output), &record.Output); err != nil {
		return RunRecord{}, fmt.Errorf("decode runbook run output: %w", err)
	}
	return record, nil
}
...
func scanEvent(row interface{ Scan(...any) error }) (EventRecord, error) {
	...
	if err := json.Unmarshal([]byte(data), &record.Data); err != nil {
		return EventRecord{}, fmt.Errorf("decode runbook event data: %w", err)
	}
	return record, nil
}
...
func scanNodeState(row interface{ Scan(...any) error }) (NodeStateRecord, error) {
	...
	if err := json.Unmarshal([]byte(output), &record.Output); err != nil {
		return NodeStateRecord{}, fmt.Errorf("decode runbook node state output: %w", err)
	}
	return record, nil
}
