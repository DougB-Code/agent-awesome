// This file persists runtime-observed runbook contract shapes.
package store
...
func (s *Store) UpsertObservedContract(ctx context.Context, record ObservedContractRecord) error {
	...
	_, err = s.db.ExecContext(ctx, `INSERT INTO runbook_observed_contracts
		...
		DO UPDATE SET occurrences = runbook_observed_contracts.occurrences + excluded.occurrences,
...
func (s *Store) ListObservedContracts(ctx context.Context, filter ObservedContractFilter) ([]ObservedContractRecord, error) {
	...
	query := `SELECT definition_id, node_id, tool_id, shape_hash, occurrences, contract_json, observed_fields_json, first_seen_at, last_seen_at FROM runbook_observed_contracts`
	if len(clauses) > 0 {
