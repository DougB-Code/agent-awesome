...
func Apply(spec Spec, input envelope.Envelope) (envelope.Envelope, []envelope.Diagnostic) {
	input.Normalize()
	output := envelope.New(input.Meta.RunbookRunID, "", input.Meta.Attempt, map[string]any{})
...
func evalBool(expr string, ctx *evalContext) (bool, error) {
	...
	return runbookexpr.Truthy(value), nil
}
...
func evalCELExpr(expression string, ctx *evalContext) (any, error) {
	return runbookexpr.Evaluate(expression, runbookexpr.VariablesFromEnvelopes(ctx.input, *ctx.output, ctx.locals))
}
