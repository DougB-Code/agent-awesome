// This file tests durable state-machine runbook runtime behavior.
package runtime
...
`)
	...
	started, err := service.StartRunbook(ctx, "approval", map[string]any{"subject": "hello"})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
...
// TestOpenSkipsInvalidDefinitionsWhenConfigured verifies embedded hosts can start while invalid runbook files are quarantined.
...
// TestDeleteDraftRemovesMirroredRunbookFile verifies deleting a runbook file removes it from disk.
func TestDeleteDraftRemovesMirroredRunbookFile(t *testing.T) {
	ctx := context.Background()
	definitionsDir := t.TempDir()
	service, err := Open(ctx, Config{
		DefinitionsDir: definitionsDir,
...
`)
	...
	if len(defs) != 1 || defs[0].ID != "copied_runbook" {
		t.Fatalf("ListDefinitions() = %#v, want copied_runbook", defs)
	}
	drafts, err := service.ListDrafts(ctx)
	if err != nil {
		t.Fatalf("ListDrafts() error = %v", err)
	}
	if len(drafts) != 1 || drafts[0].ID != "draft_copied_runbook" {
		t.Fatalf("ListDrafts() = %#v, want draft_copied_runbook", drafts)
	}
	if err := service.DeleteDraft(ctx, "draft_copied_runbook"); err != nil {
...
`)
	...
	if len(drafts) != 1 || drafts[0].ID != "draft_new_runbook" {
		t.Fatalf("ListDrafts() = %#v, want only draft_new_runbook", drafts)
	}
	if drafts[0].Kind != draftKindRunbook {
		t.Fatalf("draft kind = %q, want runbook", drafts[0].Kind)
	}
...
// TestUpdateRunbookDraftPreservesStateMachineBodyKind verifies authoring kind does not rewrite runtime kind.
func TestUpdateRunbookDraftPreservesStateMachineBodyKind(t *testing.T) {
	ctx := context.Background()
	definitionsDir := t.TempDir()
	service, err := Open(ctx, Config{
		DefinitionsDir: definitionsDir,
	...
	draft, err := service.CreateDraft(ctx, DraftRequest{
		...
		Name: "State Machine Runbook",
		Body: map[string]any{
			"kind":    definition.KindStateMachine,
			"id":      "state_machine_runbook",
			"initial": "start",
			"states": []any{
	...
	updated, err := service.UpdateDraft(ctx, draft.ID, DraftRequest{
		Kind:        draftKindRunbook,
		Name:        "Renamed State Machine Runbook",
	...
	if updated.Kind != draftKindRunbook {
		t.Fatalf("draft kind = %q, want runbook", updated.Kind)
	}
...
// TestStartRunbookReloadsCopiedDefinition verifies copied runbooks can run before the UI refreshes.
func TestStartRunbookReloadsCopiedDefinition(t *testing.T) {
	ctx := context.Background()
	definitionsDir := t.TempDir()
	service, err := Open(ctx, Config{
		DefinitionsDir: definitionsDir,
...
`)
	...
	started, err := service.StartRunbook(ctx, "runnable_copy", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	if started.DefinitionID != "runnable_copy" {
		t.Fatalf("StartRunbook() definition = %q, want runnable_copy", started.DefinitionID)
	}
	eventually(t, func() bool {
		run, err := service.Status(ctx, started.ID)
		return err == nil && run.Status == statusWaiting
...
// TestRunSetupStartsRunbookWithMergedInput verifies reusable setup input is applied.
func TestRunSetupStartsRunbookWithMergedInput(t *testing.T) {
	ctx := context.Background()
	definitionsDir := t.TempDir()
...
`)
	...
	started, err := service.StartRunbook(ctx, "hierarchical_flow", map[string]any{"ready": true})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	started, err := service.StartRunbook(ctx, "transition_flow", map[string]any{"ready": true})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	started, err := service.StartRunbook(ctx, "decision_transition_flow", map[string]any{"auto": true})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	started, err := service.StartRunbook(ctx, "node_output_route_flow", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	started, err := service.StartRunbook(ctx, "waiting_flow", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	eventually(t, func() bool {
  ...
  - id: publish
    on_entry:
      - id: publish_change
        uses: data.assert
        with:
          path: runbook_input.publish_allowed
          mode: equals
          value: true
  - id: blocked
    on_entry:
      - id: blocked_gate
        uses: data.assert
        with:
          path: runbook_input.manual_repair
          mode: exists
`)
	service := openTestService(t, ctx, definitionsDir, "")
	defer service.Close()

	started, err := service.StartRunbook(ctx, "blocked_publish", map[string]any{"ready": false, "publish_allowed": true})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
...
// TestRunbookEventsRedactSignalPayload verifies audit events do not expose credentials.
func TestRunbookEventsRedactSignalPayload(t *testing.T) {
	ctx := context.Background()
	definitionsDir := t.TempDir()
...
`)
	...
	started, err := service.StartRunbook(ctx, "redact_signal", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	eventually(t, func() bool {
...
`)
	...
	started, err := service.StartRunbook(ctx, "tool_runbook", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	for i := 0; i < 3; i++ {
		started, err := service.StartRunbook(ctx, "observe_contract", map[string]any{})
		if err != nil {
			t.Fatalf("StartRunbook() error = %v", err)
		}
		waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
func TestStateMachineLLMActionRequiresStructuredSchema(t *testing.T) {
	ctx := context.Background()
	llm := &recordingRunbookLLM{response: map[string]any{
		"status": "succeeded",
  ...
  - id: classify
    on_entry:
      - id: classify
        type: llm
        with:
          prompt: "Classify ${body.value.runbook_input.subject}"
          output_schema:
            type: object
            required:
              - status
...
`)
	...
	started, err := service.StartRunbook(ctx, "llm_runbook", map[string]any{"subject": "Invoice", "api_token": "raw-secret"})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	started, err := service.StartRunbook(ctx, "rate_limit_runbook", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusFailed)
...
`)
	...
	started, err := service.StartRunbook(ctx, "sandbox_mismatch", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusFailed)
...
`)
	...
	start := time.Now()
	started, err := service.StartRunbook(ctx, "retry_runbook", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
`)
	...
	started, err := service.StartRunbook(ctx, "resume_runbook", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusFailed)
	if _, err := service.store.DB().ExecContext(ctx, `DELETE FROM runbook_node_states WHERE run_id = ? AND state_id = 'second'`, started.ID); err != nil {
...
`)
	...
	started, err := service.StartRunbook(ctx, "gated_runbook", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusSucceeded)
...
kind: state_machine
id: failed_gate_runbook
name: Failed Gate Runbook
initial: assert_plan
states:
  - id: assert_plan
    on_entry:
...
`)
	...
	started, err := service.StartRunbook(ctx, "failed_gate_runbook", map[string]any{"status": "rejected"})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusFailed)
...
`)
	...
	started, err := service.StartRunbook(ctx, "approval_gate", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	eventually(t, func() bool {
    ...
    on_entry:
      - id: gate
        ...
        runtime:
          max_input_bytes: 10
        with:
          path: runbook_input.payload
          mode: exists
`)
	service := openTestService(t, ctx, definitionsDir, toolServer.URL+"/api/context")
	defer service.Close()

	started, err := service.StartRunbook(ctx, "input_limit", map[string]any{"payload": "this input is too large"})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusFailed)
...
func TestStateMachineRuntimeMaxArtifactBytesBlocksOversizedOutput(t *testing.T) {
	...
	toolServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(w, map[string]any{"structuredContent": map[string]any{
			"meta": map[string]any{
				"schema_ref": "aa.runbook.envelope.v1",
...
`)
	...
	started, err := service.StartRunbook(ctx, "artifact_limit", map[string]any{})
	if err != nil {
		t.Fatalf("StartRunbook() error = %v", err)
	}
	waitForRunStatus(t, ctx, service, started.ID, statusFailed)
...
// openTestService opens a runbook service for runtime tests.
...
// writeTestDefinition writes one YAML runbook definition for a runtime test.
...
// eventually waits for asynchronous runbook execution to reach a condition.
...
// waitForRunStatus waits for a runbook run to reach one status and reports history on failure.
...
// recordingRunbookLLM captures schema-constrained model requests for runtime tests.
type recordingRunbookLLM struct {
	requests []actions.LLMRequest
	response map[string]any
}

// GenerateRunbookJSON records a model request and returns fixed JSON output.
func (l *recordingRunbookLLM) GenerateRunbookJSON(_ context.Context, req actions.LLMRequest) (map[string]any, error) {
