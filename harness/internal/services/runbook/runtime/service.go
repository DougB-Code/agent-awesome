// This file implements the runbook orchestration service.
package runtime
...
// Service owns runbook definitions, persistence, and execution.
type Service struct {
	cfg                Config
	store              *store.Store
...
// Open creates a runbook service and loads declarative definitions.
func Open(ctx context.Context, cfg Config) (*Service, error) {
	registry := actions.NewRegistry()
	runbookStore, err := store.Open(ctx, cfg.DatabasePath)
	if err != nil {
		return nil, err
	}
...
func (s *Service) reloadDefinitions(ctx context.Context) error {
	...
	for _, warning := range warnings {
		log.Printf("runbook definition skipped: %s: %s", warning.Path, warning.Message)
	}
...
// loadDefinitions loads runbook definition files using configured strictness.
...
// definitionFileSnapshot hashes deployable runbook definition files in stable order.
func (s *Service) definitionFileSnapshot() (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("read runbook definitions directory %q: %w", trimmed, err)
	}
	...
	for _, path := range paths {
		...
		if err != nil {
			return "", fmt.Errorf("read runbook definition %q: %w", path, err)
		}
...
// StartRunbook creates a run and begins execution in the background.
func (s *Service) StartRunbook(ctx context.Context, definitionID string, input map[string]any) (store.RunRecord, error) {
	if err := s.syncDefinitionsFromDisk(ctx); err != nil {
		return store.RunRecord{}, err
	}
	def, ok := s.DescribeDefinition(definitionID)
	if !ok {
		return store.RunRecord{}, fmt.Errorf("runbook definition %q not found", definitionID)
	}
	runID, err := newRunID()
	if err != nil {
		return store.RunRecord{}, err
	...
	_ = s.appendEvent(ctx, run.ID, "run_started", "runbook run started", map[string]any{"definition_id": def.ID})
	go s.executeRun(context.Background(), run.ID)
	return s.store.GetRun(ctx, run.ID)
}

// Status returns one runbook run by id.
...
// History returns durable events for one runbook run.
...
func (s *Service) Cancel(ctx context.Context, runID string) (store.RunRecord, error) {
	...
	_ = s.appendEvent(ctx, run.ID, "run_canceled", "runbook run canceled", nil)
	return s.store.GetRun(ctx, run.ID)
}
...
func (s *Service) ResumeActiveRuns(ctx context.Context) error {
	...
	for _, run := range runs {
		_ = s.appendEvent(ctx, run.ID, "run_resumed", "runbook run resumed after startup", nil)
		go s.executeRun(context.Background(), run.ID)
	}
	return nil
}

// Signal applies a user or system signal to a waiting or running runbook.
func (s *Service) Signal(ctx context.Context, runID string, signal string, payload map[string]any) (store.RunRecord, error) {
	...
	if !ok {
		return store.RunRecord{}, fmt.Errorf("runbook definition %q not loaded", run.DefinitionID)
	}
	...
	if err := s.appendEvent(ctx, run.ID, "signal_received", "runbook signal received", map[string]any{"signal": signal, "payload": payload}); err != nil {
		return store.RunRecord{}, err
	}
	if !definition.HasStateMachine(def) {
		return store.RunRecord{}, fmt.Errorf("runbook definition %q is not executable", def.ID)
	}
