// This file starts simple daily cron schedules for loaded runbooks.
package runtime
...
// StartScheduler starts the lightweight runbook scheduler loop.
...
// fireDueSchedules starts runbooks whose simple daily cron is due.
func (s *Service) fireDueSchedules(ctx context.Context, now time.Time, last map[string]string) {
	...
	for _, id := range defs {
		...
		last[id] = key
		_, _ = s.StartRunbook(ctx, id, map[string]any{"scheduled_at": now.UTC().Format(time.RFC3339)})
	}
}
