// This file schedules and invokes runbook state-machine runs.
package runtime
...
func (s *Service) executeRun(ctx context.Context, runID string) {
	...
	if !ok {
		s.failRun(ctx, run, fmt.Errorf("runbook definition %q not loaded", run.DefinitionID))
		return
	}
	if !definition.HasStateMachine(def) {
		s.failRun(ctx, run, fmt.Errorf("runbook definition %q is not a state machine", def.ID))
		return
	}
