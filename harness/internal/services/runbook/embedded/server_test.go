// This file tests the in-process runbook host surface.
package embedded
...
// TestStartServesHealth verifies embedded runbook exposes the HTTP routes.
func TestStartServesHealth(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	server, err := Start(ctx, Config{
		ListenAddress:         "127.0.0.1:0",
		DefinitionsDir:        filepath.Join(t.TempDir(), "runbooks"),
