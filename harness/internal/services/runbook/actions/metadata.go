// This file defines built-in runbook action metadata and manifests.
package actions
...
func MetadataFor(name string) Metadata {
	action := Metadata{
		...
		InputContracts: []string{"aa.runbook.action_input.v1"},
	}
	switch action.Name {
	case "tool.call":
	...
	case "data.assert":
		...
		action.Description = "Gate runbook progression on deterministic input data checks."
	...
	case "data.defaults":
		...
		action.InputContracts = []string{"aa.runbook.action_input.v1"}
		action.OutputContracts = []string{"aa.runbook.action_output.v1"}
	case "decision.route":
	...
	case "runbook.run":
		action.Label = "Run Runbook"
		action.Description = "Start a nested runbook definition."
		action.Risk = "runbook"
		action.InputSchema = map[string]any{"type": "object", "required": []any{"runbook"}, "properties": map[string]any{"runbook": map[string]any{"type": "string"}, "input": map[string]any{"type": "object"}}}
		...
		action.InputContracts = []string{"aa.runbook_run_request.v1"}
		action.OutputContracts = []string{"aa.runbook_run_result.v1"}
	case "runbook.signal":
		action.Label = "Signal Runbook"
		action.Description = "Emit a runbook signal."
		action.Risk = "runbook"
		...
		action.InputContracts = []string{"aa.runbook_signal_request.v1"}
		action.OutputContracts = []string{"aa.runbook_signal_result.v1"}
	case "human.request":
