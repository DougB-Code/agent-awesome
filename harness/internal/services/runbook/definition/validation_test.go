// This file tests runbook state-machine validation rules.
package definition
...
// TestLoadFileAcceptsProfessionalCodingRunbook verifies the shipped runbook is deployable.
func TestLoadFileAcceptsProfessionalCodingRunbook(t *testing.T) {
	path := filepath.Join("..", "..", "..", "..", "runbooks", "professional_coding_change.yaml")
		...
		States: []StateDefinition{
			...
			},
			{
				...
				OnEntry: []NodeDefinition{
					...
					{ID: "child_call", Uses: "runbook.run"},
				},
			},
		},
	}, testCatalog{"mcp.call": true, "tool.call": true, "runbook.run": true})
