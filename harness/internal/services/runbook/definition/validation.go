// This file validates hierarchical runbook state-machine definitions.
package definition
...
// Validate checks a runbook definition for deterministic, registered behavior.
func Validate(def Definition, actions ActionCatalog) error {
	if err := validateSafeID(def.ID, "runbook id"); err != nil {
		return err
	}
	...
	return fmt.Errorf("runbook %q kind must be %q", def.ID, KindStateMachine)
}
