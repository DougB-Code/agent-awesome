// This file loads runbook definitions from YAML files.
package definition
...
// LoadDirectory reads and validates every YAML runbook definition in a directory.
...
// LoadDirectorySkippingInvalid reads valid runbook definitions and reports skipped files.
...
func loadDirectory(dir string, actions ActionCatalog, skipInvalid bool) ([]LoadedDefinition, []LoadWarning, error) {
	...
	if err != nil {
		...
		return nil, nil, fmt.Errorf("read runbook definitions directory %q: %w", trimmed, err)
	}
	...
	for _, path := range paths {
		...
		if _, ok := seen[def.Definition.ID]; ok {
			return nil, warnings, fmt.Errorf("duplicate runbook definition %q", def.Definition.ID)
		}
...
// LoadFile reads and validates one YAML runbook definition file.
func LoadFile(path string, actions ActionCatalog) (LoadedDefinition, error) {
	body, err := os.ReadFile(path)
	if err != nil {
		return LoadedDefinition{}, fmt.Errorf("read runbook definition %q: %w", path, err)
	}
	var def Definition
	decoder := yaml.NewDecoder(bytes.NewReader(body))
	decoder.KnownFields(true)
	if err := decoder.Decode(&def); err != nil {
		return LoadedDefinition{}, fmt.Errorf("decode runbook definition %q: %w", path, err)
	}
	...
	if err != nil {
		return LoadedDefinition{}, fmt.Errorf("normalize runbook definition %q: %w", path, err)
	}
