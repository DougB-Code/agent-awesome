// This file exposes user-channel-safe runbook HTTP routes for the gateway.
package transport
...
// HTTPServer serves runbook REST routes and MCP JSON-RPC.
type HTTPServer struct {
	service *runtime.Service
	mcp     *MCPServer
}

// NewHTTPServer creates the runbook HTTP transport.
func NewHTTPServer(service *runtime.Service) *HTTPServer {
	return &HTTPServer{service: service, mcp: NewMCPServer(service)}
}

// Routes builds the runbook HTTP route multiplexer.
func (s *HTTPServer) Routes() http.Handler {
	mux := http.NewServeMux()
	mux.HandleFunc("/healthz", s.healthHandler)
	mux.Handle("/mcp", s.mcp)
	mux.HandleFunc("/api/runbooks/action-types", s.actionTypesHandler)
	mux.HandleFunc("/api/runbooks/manifests", s.manifestsHandler)
	mux.HandleFunc("/api/runbooks/mappings/preview", s.mappingPreviewHandler)
	mux.HandleFunc("/api/runbooks/design/artifacts", s.designArtifactsHandler)
	mux.HandleFunc("/api/runbooks/design/suggest", s.designSuggestHandler)
	mux.HandleFunc("/api/runbooks/observed-contracts", s.observedContractsHandler)
	mux.HandleFunc("/api/runbooks/definitions", s.definitionsHandler)
	mux.HandleFunc("/api/runbooks/definitions/", s.definitionHandler)
	mux.HandleFunc("/api/runbooks/drafts", s.draftsHandler)
	mux.HandleFunc("/api/runbooks/drafts/", s.draftHandler)
	mux.HandleFunc("/api/runbooks/packages", s.packagesHandler)
	mux.HandleFunc("/api/runbooks/packages/", s.packageHandler)
	mux.HandleFunc("/api/runbooks/run-setups", s.runSetupsHandler)
	mux.HandleFunc("/api/runbooks/run-setups/", s.runSetupHandler)
	mux.HandleFunc("/api/runbooks/runs", s.runsHandler)
	mux.HandleFunc("/api/runbooks/runs/", s.runHandler)
	mux.HandleFunc("/api/runbooks/inbox", s.inboxHandler)
	return mux
}

// healthHandler reports runbook service liveness.
...
// definitionsHandler lists installed runbook definitions.
...
func (s *HTTPServer) definitionHandler(w http.ResponseWriter, r *http.Request) {
	definitionID, action := splitTail(r.URL.Path, "/api/runbooks/definitions/")
	if definitionID == "" {
	...
	if r.Method == http.MethodGet && action == "dot" {
		...
		if !ok {
			writeJSON(w, http.StatusNotFound, map[string]string{"error": "runbook definition not found"})
			return
		}
	...
	writeJSON(w, http.StatusNotFound, map[string]string{"error": "runbook definition route not found"})
}
...
// manifestsHandler lists AA-owned runbook action manifests.
...
// draftsHandler lists or creates runbook drafts.
...
func (s *HTTPServer) draftHandler(w http.ResponseWriter, r *http.Request) {
	draftID, action := splitTail(r.URL.Path, "/api/runbooks/drafts/")
	if draftID == "" {
	...
	default:
		writeJSON(w, http.StatusNotFound, map[string]string{"error": "runbook draft route not found"})
	}
}

// packagesHandler lists or imports runbook packages.
...
// packageHandler exports one runbook package.
func (s *HTTPServer) packageHandler(w http.ResponseWriter, r *http.Request) {
	packageID, action := splitTail(r.URL.Path, "/api/runbooks/packages/")
	if packageID == "" {
	...
	writeJSON(w, http.StatusNotFound, map[string]string{"error": "runbook package route not found"})
}

// runSetupsHandler lists or creates reusable runbook run setups.
...
// runSetupHandler routes reusable runbook run setup launchpad.
func (s *HTTPServer) runSetupHandler(w http.ResponseWriter, r *http.Request) {
	setupID, action := splitTail(r.URL.Path, "/api/runbooks/run-setups/")
	if setupID == "" {
	...
	default:
		writeJSON(w, http.StatusNotFound, map[string]string{"error": "runbook run setup route not found"})
	}
}

// runsHandler lists or starts runbook runs.
func (s *HTTPServer) runsHandler(w http.ResponseWriter, r *http.Request) {
	...
	run, err := s.service.StartRunbook(r.Context(), req.DefinitionID, req.Input)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
...
func (s *HTTPServer) runHandler(w http.ResponseWriter, r *http.Request) {
	path := strings.TrimPrefix(r.URL.Path, "/api/runbooks/runs/")
	...
	case r.Method == http.MethodPost && action == "cancel":
		...
		writeResult(w, map[string]any{"run": run}, err)
	default:
		writeJSON(w, http.StatusNotFound, map[string]string{"error": "runbook route not found"})
	}
}

// signalHandler decodes and applies one runbook signal.
...
// inboxHandler lists runbook items waiting for user/channel input.
...
// startRequest is the REST payload for creating a runbook run.
type startRequest struct {
	DefinitionID string         `json:"definition_id"`
	Input        map[string]any `json:"input"`
}

// signalRequest is the REST payload for signaling a runbook run.
