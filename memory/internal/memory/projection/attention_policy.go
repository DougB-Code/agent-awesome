...
func attentionReasonForTask(q domain.ExecutiveSummaryQuery, index taskIndex, task domain.Task, lane string) string {
	switch lane {
	case "protect":
		return "High-priority or scheduled work should not be casually displaced."
	case "decide":
		if task.Risk >= 0.65 {
			return "High-risk work needs your approval."
		}
		return "Risk, urgency, or priority requires human judgment."
	case "delegate":
		return "Low-risk work with concrete task context can be prepared safely."
	case "follow_up":
		return "Explicit person or follow-up metadata needs attention."
	case "monitor":
...
func attentionScore(q domain.ExecutiveSummaryQuery, index taskIndex, task domain.Task) float64 {
	...
	importance := priorityImportanceScore(task)
	...
	return roundScore(pressure*0.22 + timePressure*0.18 + risk*0.16 + importance*0.14 + urgency*0.12 + relationshipCost*0.08 + forgetting*0.08 - delegable*0.06)
}

// priorityImportanceScore derives importance from explicit task priority.
func priorityImportanceScore(task domain.Task) float64 {
...
func relationshipLoopDue(q domain.ExecutiveSummaryQuery, task domain.Task) bool {
	if task.Person == "" || task.FollowUpAt == nil {
		return false
	}
...
// safelyDelegable reports whether explicit task facts describe bounded low-risk work.
func safelyDelegable(task domain.Task) bool {
	...
	return task.Description != "" || len(task.MemoryLinks) > 0
}

// unsafeTask reports whether explicit risk metadata requires approval.
func unsafeTask(task domain.Task) bool {
	return task.Risk >= 0.65
}
...
func shouldProtect(q domain.ExecutiveSummaryQuery, task domain.Task) bool {
	...
	if priorityImportanceScore(task) >= 0.75 && (task.Risk >= 0.35 || task.EstimateMinutes >= 60) {
		return true
	}
